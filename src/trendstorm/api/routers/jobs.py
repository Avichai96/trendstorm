...
@router.get(
    ...
    summary="Fetch the analysis for a completed job",
)
async def get_job_analysis(
    ...
    """Return the analysis associated with a completed job.

    Returns 404 while the job is still running (analysis not yet written).
    """
    if not is_valid_id(job_id):
        raise NotFoundError(f"Job {job_id} not found")
    ...
    job = await job_repo.get(request.state.tenant_id, job_id)
    if job is None:
        raise NotFoundError(f"Job {job_id} not found")
    ...
    analysis = await analysis_repo.get_for_job(request.state.tenant_id, job_id)
    if analysis is None:
        raise NotFoundError(f"Analysis for job {job_id} not yet available")

    return AnalysisResponse(
        id=analysis.id,
        job_id=job_id,
        summary=analysis.summary,
        validator_score=analysis.validator_score,
        validator_passed=analysis.validator_passed,
        refinement_loops=analysis.refinement_loops,
